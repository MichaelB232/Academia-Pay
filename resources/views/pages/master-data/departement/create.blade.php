...
@section('content')
    <div class="max-w-md mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-5 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-sm font-bold text-gray-800">Tambah Departement Baru</h3>
        ...
        <form action="{{ route('departement.store') }}" method="POST" onsubmit="disableSubmitButton()" class="p-6 space-y-4">
            @csrf

            <div>
                <label class="block text-[10px] font-bold text-gray-400 tracking-wider uppercase mb-1">Nama
                    Departement</label>
                <input type="text" name="nama_departement" value="{{ old('nama_departement') }}" required
                    class="w-full bg-gray-50/50 border @error('nama_departement') border-red-500 focus:border-red-500 @else border-gray-200 focus:border-[#0a855c] @enderror rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:bg-white transition-all">
